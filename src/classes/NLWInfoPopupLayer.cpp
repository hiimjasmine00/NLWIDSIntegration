...
bool NLWInfoPopupLayer::init(NLWRating* rating) {
    ...
    auto enjoyment = rating->enjoyment;

    auto name = CCLabelBMFont::create(rating->name.c_str(), "bigFont.fnt");
    name->setPosition({ enjoyment == -1 ? 15.0f : 40.0f, 165.0f });
    ...
    creator->setID("creator-label");
    m_mainLayer->addChild(creator);

    auto tierMenu = CCMenu::create();
    tierMenu->setPosition({ 310.0f, 165.0f });
    tierMenu->setID("tier-menu");
    m_mainLayer->addChild(tierMenu);

    auto& tierStr = rating->tier;
    auto type = rating->type;

    auto tier = CCLabelBMFont::create(type == NLWRatingType::Pending ? tierStr.c_str() : fmt::format("{} Tier", tierStr).c_str(), "bigFont.fnt");
    tier->setColor(type == NLWRatingType::Pending ? ccColor3B { 255, 255, 255 } : NLWIntegration::getTierColor(tierStr));
    ...
    tierMenuItem->setID("tier-button");
    ...
    background->setID("description-background");
    ...
    description->setID("description-label");
    ...
    openBtn->setID("sheet-button");
    m_buttonMenu->addChild(openBtn);

    if (enjoyment != -1) {
        ccColor3B enjoymentColor = { 255, 255, 255 };
        if (enjoyment > 90) enjoymentColor = { 0, 255, 254 };
        else if (enjoyment > 75) enjoymentColor = { 179, 215, 170 };
        else if (enjoyment > 60) enjoymentColor = { 255, 229, 159 };
        else if (enjoyment > 40) enjoymentColor = { 254, 203, 160 };
        else if (enjoyment > 0) enjoymentColor = { 240, 153, 154 };

        auto enjSprite = CCSprite::create("NLW_button_white.png"_spr);
        enjSprite->setPosition({ 25.0f, 165.0f });
        enjSprite->setScale(0.5f);
        enjSprite->setColor(enjoymentColor);
        enjSprite->setID("enjoyment-sprite");
        m_mainLayer->addChild(enjSprite);

        auto enj = CCLabelBMFont::create(fmt::to_string(enjoyment).c_str(), "bigFont.fnt");
        enj->setPosition(enjSprite->getContentSize() / 2.0f);
        enj->setScale(0.83f);
        enj->setID("enjoyment-label");
...
void NLWInfoPopupLayer::openTierLevels(CCObject* sender) {
    ...
    auto insane = m_rating->insane;
    for (auto& rating : NLWIntegration::ratings) {
